...
import type { GameScreen, ItemId, CodePanelMode } from '../types/game';
...
export function useGameEngine() {
  ...
  const startGame = useCallback((container: HTMLElement) => {
    ...
    engine.events.on('showCodePanel', (title: unknown, subtitle: unknown, mode: unknown) => {
      setState((s) => ({
        ...s,
        codePanelOpen: true,
        codePanelTitle: title as string,
        codePanelSubtitle: subtitle as string,
        codePanelMode: (mode as CodePanelMode) || 'alpha',
        codeDisplay: '',
        codeResultText: '',
        codeResultColor: '',
      }));
